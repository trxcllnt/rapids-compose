...
# # copy in python env, cudf, and cugraph
# COPY --from=cudf_base /opt/rapids/cudf /opt/rapids/cudf
# COPY --from=cugraph_base /opt/rapids/cugraph /opt/rapids/cugraph
# # copy in required cuda libs and binaries
# COPY --from=cuda_base ${CUDA_HOME}/bin/nvcc ${CUDA_HOME}/bin/nvcc
...
# # RUN conda env update -n base --no-update-deps \
# #     -f /opt/rapids/cudf/conda/environments/cudf_dev_cuda${CUDA_SHORT_VERSION}.yml \
# #     -f /opt/rapids/cugraph/conda/environments/cugraph_dev_cuda${CUDA_SHORT_VERSION}.yml \
# #  && cd /opt/rapids/cudf/python && python setup.py install \
...
#  && pip install --no-cache-dir numba cython \
#  && cd /opt/rapids/cudf/python && python setup.py install \
#  && cd /opt/rapids/cugraph/python && python setup.py install \
#  # Symlink the base conda env's python as global python and python3.
#  # This lets us use conda without having to "source activate env" everywhere.
#  # Conda envs are kind of redundant since we're already in a virtual machine.
#  && ln -s $(which python${PYTHON_VERSION}) /usr/local/bin/python \
#  && ln -s $(which python${PYTHON_VERSION}) /usr/local/bin/python3 \
#  # Smoke test
#  && python -c "from cudf.dataframe import DataFrame" \
...
# # Add tini to reap container subprocesses on exit
# RUN curl -L https://github.com/krallin/tini/releases/download/${TINI_VERSION}/tini -o /usr/bin/tini && chmod +x /usr/bin/tini \
#  && mkdir -p /home/rapids ${PTVSD_LOG_DIR} \
#  # Symlink dirs to root for compatibility with existing scripts
#  && ln -s /opt/rapids /rapids \
#  && ln -s /opt/rapids/cudf /cudf \
